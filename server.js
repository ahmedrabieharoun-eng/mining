/**
 * ========================================================================
 *  server.js — Backend كامل لمشروع Telegram Mini App (OreRush Bot)
...
const DEFAULT_CONFIG = {
  ...
  tonConversionRate: 10000,    // 10,000 ORE = 1 TON
  ...
  try {
    ...
    if (!existingRef) {
      ...
      await sendTelegramMessage(env, config.botToken || '', referrerId,
        `👥 New referral joined!\n\n👤 ${user.firstName || user.username || 'A user'} opened OreRush Bot with your link.\n\n⏳ They need to watch 10 ads before you get paid.\n💎 Your reward: +${Number(reward).toLocaleString('en-US')} ORE — credited once, as soon as they finish`);
    }
...
async function handleRequestWithdrawal(env, ctx) {
  ...
  const withdrawalNotifyMessage =
    ...
    `💜 Thank you for using OreRush Bot\n` +
    `━━━━━━━━━━━━━━━`;
...
// وحساب رصيد ORE الحالي بدل KV المستخدم في التطبيق المنفصل.
...
// Convert ORE to TON. No external payment or blockchain verification is used.
async function handleConvertPmtToTon(env, ctx) {
  ...
  if (pmtAmount > pmtBalance) return fail('Insufficient ORE balance.');
...
async function handleFetch(request, env) {
    ...
    const botUsername = config.botUsername || env.BOT_USERNAME || 'OreRush_Bot';
...
// ════════════════════════════════════════════════════════════════════
//  نقطة الدخول لـ Cloudflare Workers (ES Modules format)
//  fetch: كل طلب HTTP عادي بيتحول لـ handleFetch فوق زي ما هو من غير
//         أي تغيير في منطق الراوتس أو الهاندلرز.
//  scheduled: بديل الـ setInterval اللي كان شغال على Railway — Cloudflare
//         Workers بيدعم Cron Triggers بشكل أصلي (لازم تتضاف من لوحة
//         التحكم Workers & Pages > اسم الـ Worker > Settings > Triggers
//         > Cron Triggers، مثلاً كل دقيقة: * * * * *).
// ════════════════════════════════════════════════════════════════════
export default {
  ...
  async scheduled(controller, env, ctx) {
    try {
      ...
      await ensureWeeklyContestUpToDate(env, config);
    } catch (err) {
